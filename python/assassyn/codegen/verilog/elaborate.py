...
REGFILE_MARKER = "// Auto-generated by assassyn.codegen.verilog (external regfile)"
GENERATED_ALIAS_BASES = ("fifo", "trigger_counter")
...
def _remove_stale_generated_alias_resources(path: Path) -> None:
    """Remove generated helper aliases from prior elaborations in this directory."""
    for base in GENERATED_ALIAS_BASES:
        pattern = re.compile(rf"^\s*module\s+{re.escape(base)}_\d+\b", re.MULTILINE)
        for sv_file in path.glob(f"{base}_*.sv"):
            try:
...
def generate_register_file_sv_files(sys: SysBuilder, path: Path) -> List[str]:
    ...
    for arr in sys.arrays:
        ...
        if metadata is None:
            num_write_ports = physical_write_port_count(metadata, arr)
            num_read_ports = 0
        else:
            num_write_ports = physical_write_port_count(metadata, arr)
        ...
        readonly_initialized = depth > 1 and num_write_ports == 0 and initializer is not None
        synthesis_blackbox = readonly_initialized or synthesis_blackbox_array(arr)
        if initializer is not None and len(initializer) != depth:
            msg = (
        ...
        module_decl.extend(port_lines)
        module_decl.append(");")

        lines: List[str] = []
        lines.append(REGFILE_MARKER)
        if synthesis_blackbox:
            lines.append("`ifdef SYNTHESIS")
            lines.append("(* blackbox *)")
            lines.extend(module_decl)
...
def elaborate(sys: SysBuilder, **kwargs) -> str:
    ...
    _remove_stale_generated_alias_resources(path)
